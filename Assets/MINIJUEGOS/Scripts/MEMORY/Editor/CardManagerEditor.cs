...
public class CardManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        _pairAmount.intValue = Mathf.CeilToInt(tmp);

        if (_pairAmount.intValue > spriteAmount)
        {
            EditorGUILayout.HelpBox("Demasiados pares para la cantidad de sprites", MessageType.Error);
        }

        if (_width.intValue < 0) _width.intValue = 0;
        if (_height.intValue < 0) _height.intValue = 0;
